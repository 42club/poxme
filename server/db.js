...
const _upsertMiniAppPreferences = db.prepare(`
  INSERT INTO mini_app_preferences (user_id, settings_json, state_json, created_at, updated_at)
  VALUES (@user_id, @settings_json, @state_json, @created_at, @updated_at)
  ON CONFLICT(user_id) DO UPDATE SET
    settings_json=excluded.settings_json,
    state_json=excluded.state_json,
    updated_at=excluded.updated_at
`)
const _getMiniAppPreferences = db.prepare(`SELECT * FROM mini_app_preferences WHERE user_id = ? LIMIT 1`)
...
function _parseMiniAppPreferences(row) {
  if (!row) return null
  return {
    userId: row.user_id,
...
export function getMiniAppPreferences(userId) {
  return _parseMiniAppPreferences(_getMiniAppPreferences.get(userId))
}

export function upsertMiniAppPreferences({ userId, settings = {}, state = {} }) {
  const existing = _getMiniAppPreferences.get(userId)
  const now = Date.now()
  _upsertMiniAppPreferences.run({
    user_id: userId,
