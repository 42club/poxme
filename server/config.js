...
const requestedSqlitePath = typeof process.env.SQLITE_PATH === 'string'
  ? process.env.SQLITE_PATH.trim()
  : ''
const defaultCorsOrigins = process.env.PUBLIC_WEB_ORIGIN
  ? [process.env.PUBLIC_WEB_ORIGIN]
  : ['http://localhost:3000', 'http://localhost:3002', 'http://localhost:5173']
const sqlitePath = dbDriver === 'postgres'
  ? (
      requestedSqlitePath && requestedSqlitePath !== ':memory:'
        ? requestedSqlitePath
        : (isProd ? 'server/data/odrob-compat.db' : ':memory:')
    )
  : (requestedSqlitePath || 'server/data/odrob.db')
...
const config = {
  ...
  telegram: {
    ...
    initDataMaxAgeSec: parseInt(process.env.TELEGRAM_INIT_DATA_MAX_AGE_SEC, 10) || 86400,
    sessionTtlMs: parseInt(process.env.TELEGRAM_SESSION_TTL_MS, 10) || (24 * 60 * 60 * 1000),
    autoProvisionWdkWallet: process.env.TELEGRAM_AUTO_PROVISION_WDK_WALLET !== 'false',
  },
