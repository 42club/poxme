...
import {
  ...
  getMiniAppPreferences as sqliteGetMiniAppPreferences,
  ...
  upsertMiniAppPreferences as sqliteUpsertMiniAppPreferences,
...
function parseMiniAppPreferences(row) {
  if (!row) return null
  return {
    userId: row.user_id,
...
export async function ensureRuntimeAuthStoreReady() {
  ...
  ensureRuntimeAuthStorePromise = (async () => {
    ...
    const bootstrapStatements = [
      ...
      `CREATE TABLE IF NOT EXISTS ${qualifiedTable('mini_app_preferences')} (
        user_id TEXT PRIMARY KEY REFERENCES ${qualifiedTable('app_users')} (id) ON DELETE CASCADE,
...
export async function getMiniAppPreferences(userId) {
  if (!POSTGRES_ENABLED) return sqliteGetMiniAppPreferences(userId)
  const result = await query(`SELECT * FROM ${qualifiedTable('mini_app_preferences')} WHERE user_id = $1 LIMIT 1`, [userId])
  return parseMiniAppPreferences(result.rows[0])
}

export async function upsertMiniAppPreferences({ userId, settings = {}, state = {} }) {
  if (!POSTGRES_ENABLED) return sqliteUpsertMiniAppPreferences({ userId, settings, state })
  const existing = await getMiniAppPreferences(userId)
  const now = Date.now()
  await query(
    `INSERT INTO ${qualifiedTable('mini_app_preferences')} (
      user_id, settings_json, state_json, created_at, updated_at
    ) VALUES ($1, $2, $3, $4, $5)
  ...
  return getMiniAppPreferences(userId)
}
