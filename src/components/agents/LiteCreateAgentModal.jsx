...
export default function LiteCreateAgentModal({ walletAddress, onClose, onCreated, inline = false }) {
  ...
  return (
    <div className={inline ? 'lt-ca-shell' : 'lt-modal-overlay'} onClick={inline ? undefined : onClose}>
      <div className={`lt-modal lt-ca-modal${inline ? ' lt-ca-modal-inline' : ''}`} onClick={(event) => event.stopPropagation()}>
        {!inline ? <button className="lt-modal-close" onClick={onClose}>✕</button> : null}
